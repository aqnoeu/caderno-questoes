...
import { System2Home, System2Cadastro, System2Filter, System2Content } from "./system2";
import "./styles.css";
...
function App() {
  ...
  return (
    <main className="app-shell">
      ...
      {area === "user" ? <>
        {tab === "inicio" && <UserHome questions={questions} userId={session.user.id} name={profile?.display_name} email={profile?.email || session.user.email} supabase={supabase} goStudy={() => setTab("estudos")} goPerformance={() => setTab("desempenho")} goChallenge={() => setTab("desafio")} />}
        ...
        {tab === "desafio" && <DailyChallenge supabase={supabase} onBack={() => setTab("inicio")} onRanking={() => setTab("desempenho")} />}
      ...
      </> : isAdmin ? <section className="admin-layout"><aside className="admin-sidebar"><button className="admin-back" onClick={() => openUser("inicio")}>← Área do aluno</button><h2>Painel administrativo</h2><small>Gestão do acervo</small>{[["overview", "Visão geral"], ["cadastro", "Cadastro"], ["questoes", "Questões"], ["essay-cadastro", "Cadastro de discursivas"], ["essay-library", "Discursivas cadastradas"], ["desafios", "Desafios"], ["creditos", "Créditos IA"], ["sistema2", "Sistema 2.0"], ["sistema2-filter", "Filtro de IA 2.0"], ["sistema2-content", "Controle 2.0"]].map(([id, label]) => <button key={id} className={["sistema2", "sistema2-cadastro", "sistema2-filter", "sistema2-content"].includes(adminTab) && id === "sistema2" || adminTab === id ? "active" : ""} onClick={() => setAdminTab(id)}>{label}</button>)}</aside><div className="admin-content">{adminTab === "overview" && <AdminOverview questions={questions} supabase={supabase} />}{adminTab === "cadastro" && cadastroContent}{adminTab === "questoes" && <QuestionLibrary questions={questions} remove={remove} toggleHidden={toggleHidden} updateQuestion={updateQuestion} supabase={supabase} />}{adminTab === "essay-cadastro" && <EssayAdmin supabase={supabase} />}{adminTab === "essay-library" && <EssayLibrary supabase={supabase} />}{adminTab === "desafios" && <ChallengeAdmin supabase={supabase} />}{adminTab === "creditos" && <CorrectionCreditsAdmin supabase={supabase} />}{adminTab === "sistema2" && <System2Home openCadastro={() => setAdminTab("sistema2-cadastro")} openFilter={() => setAdminTab("sistema2-filter")} openContent={() => setAdminTab("sistema2-content")} />}{adminTab === "sistema2-cadastro" && <System2Cadastro supabase={supabase} onBack={() => setAdminTab("sistema2")} />}{adminTab === "sistema2-filter" && <System2Filter supabase={supabase} onBack={() => setAdminTab("sistema2")} openContent={() => setAdminTab("sistema2-content")} />}{adminTab === "sistema2-content" && <System2Content supabase={supabase} onBack={() => setAdminTab("sistema2")} />}</div></section> : <section className="card"><h2>Acesso restrito</h2><p>Esta área é exclusiva para administradores.</p><button onClick={() => openUser("inicio")}>Voltar ao início</button></section>}
    </main>
  );
}
...
function LineIcon({ name, size = 22 }) {
  ...
  return <svg className="line-icon" width={size} height={size} viewBox="0 0 24 24" aria-hidden="true" {...props}>
    ...
    {name === "bolt" && <path d="m13.5 2.8-8 11h5.2l-.2 7.4 8-11h-5.2z" />}
    {name === "timer" && <><circle cx="12" cy="13" r="7" /><path d="M12 13V9m0-6v3m-3-3h6" /></>}
    {name === "bars" && <><path d="M4.5 19.5h15" /><path d="M6.5 17v-4.5h3V17M11.5 17V8.5h3V17M16.5 17V5h3V17" /></>}
    {name === "target" && <><circle cx="9.5" cy="14.5" r="6.5" /><circle cx="9.5" cy="14.5" r="2" /><path d="M9.5 14.5 16.9 7.1" /><path d="m10.2 13.8-2.1.8.8-2.1" /><path fill="currentColor" stroke="none" d="m19.6 4.4-5 2 2.9 2.9z" /></>}
...
function UserHome({ questions, userId, name: profileName, email, supabase, goStudy, goPerformance, goChallenge }) {
  ...
  return <section className="user-home">
    ...
    <div className="home-shortcuts"><DailyChallengeShortcut supabase={supabase} onOpen={goChallenge} /><button className="home-shortcut review" onClick={goStudy}><span><LineIcon name="x" size={27} /></span><div><b>Revisar erros</b><small>Pratique o que você ainda precisa reforçar.</small></div><i>›</i></button><button className="home-shortcut difficult" onClick={goStudy}><span><LineIcon name="bars" size={27} /></span><div><b>Questões difíceis</b><small>Desafie-se com questões mais complexas.</small></div><i>›</i></button></div>
...
const formatChallengeTime = (value) => {
  const milliseconds = Number(value || 0), minutes = Math.floor(milliseconds / 60000), seconds = Math.floor((milliseconds % 60000) / 1000), cents = Math.floor((milliseconds % 1000) / 10);
  return `${minutes ? `${String(minutes).padStart(2, "0")}:` : ""}${String(seconds).padStart(2, "0")},${String(cents).padStart(2, "0")}s`;
};
function DailyChallengeShortcut({ supabase, onOpen }) {
  const [data, setData] = React.useState(null);
  React.useEffect(() => { let live = true; supabase.rpc("get_today_daily_challenge").then(({ data: value }) => { if (live) setData(value || { available: false }); }); return () => { live = false; }; }, [supabase]);
  if (data && !data.available) return <button className="home-shortcut challenge-off" onClick={onOpen}><span><LineIcon name="bolt" size={27} /></span><div><b>Desafio diário</b><small>O desafio de hoje será publicado em breve.</small></div><i>›</i></button>;
  const attempt = data?.attempt;
  return <button className="home-shortcut challenge-shortcut" onClick={onOpen}><span><LineIcon name="bolt" size={27} /></span><div><b>{attempt?.status === "completed" ? "Desafio concluído" : "Desafio diário"}</b><small>{attempt?.status === "completed" ? `${formatChallengeTime(attempt.elapsed_ms)} · Ver classificação` : attempt ? "Continue sua única tentativa válida." : "5 questões. Quanto tempo você leva?"}</small></div><i>›</i></button>;
}
function DailyChallengeRanking({ supabase, compact = false }) {
  const [ranking, setRanking] = React.useState(null);
  React.useEffect(() => { let live = true; supabase.rpc("get_daily_challenge_ranking").then(({ data }) => { if (live) setRanking(data); }); return () => { live = false; }; }, [supabase]);
  if (!ranking?.entries?.length) return compact ? null : <section className="challenge-ranking empty"><h2>Ranking dos desafios</h2><p>Conclua o desafio de hoje para entrar na classificação.</p></section>;
  return <section className={`challenge-ranking ${compact ? "compact-ranking" : ""}`}><div><span className="eyebrow">DESAFIO DIÁRIO</span><h2>{compact ? "Classificação de hoje" : "Ranking dos desafios"}</h2></div><ol>{ranking.entries.slice(0, compact ? 5 : 10).map((entry) => <li key={`${entry.position}-${entry.name}`} className={entry.position <= 3 ? `top top-${entry.position}` : ""}><b>{entry.position}º</b><span>{entry.name}</span><em>{formatChallengeTime(entry.elapsed_ms)}</em></li>)}</ol>{ranking.mine && ranking.mine.position > (compact ? 5 : 10) && <p className="my-ranking">Sua posição: <b>{ranking.mine.position}º</b> · {formatChallengeTime(ranking.mine.elapsed_ms)}</p>}</section>;
}
function DailyChallenge({ supabase, onBack, onRanking }) {
  const [data, setData] = React.useState(null), [started, setStarted] = React.useState(false), [feedback, setFeedback] = React.useState(null), [message, setMessage] = React.useState(""), [now, setNow] = React.useState(Date.now()), [busy, setBusy] = React.useState(false);
  const swipeStart = React.useRef(null);
  const load = React.useCallback(async () => { const { data: value, error } = await supabase.rpc("get_today_daily_challenge"); if (error) setMessage(error.message); else setData(value || { available: false }); }, [supabase]);
  React.useEffect(() => { load(); }, [load]);
  const attempt = data?.attempt, questions = data?.questions || [], current = questions[attempt?.current_position || 0];
  const running = started && attempt && attempt.status !== "completed";
  React.useEffect(() => { if (!running) return undefined; const timer = window.setInterval(() => setNow(Date.now()), 40); return () => window.clearInterval(timer); }, [running]);
  const elapsed = attempt?.started_at ? Math.max(0, now - new Date(attempt.started_at).getTime()) : 0;
  React.useEffect(() => { if (!feedback?.next_allowed_at) return undefined; const target = new Date(feedback.next_allowed_at).getTime(); const timer = window.setInterval(() => setNow(Date.now()), 120); return () => window.clearInterval(timer); }, [feedback?.next_allowed_at]);
  const penaltyLeft = feedback?.next_allowed_at ? Math.max(0, Math.ceil((new Date(feedback.next_allowed_at).getTime() - now) / 1000)) : 0;
  React.useEffect(() => { if (!feedback || penaltyLeft > 0) return; (async () => { if (feedback.awaiting_completion) { const { error } = await supabase.rpc("finish_daily_challenge_attempt", { p_attempt_id: attempt.id }); if (error) return setMessage(error.message); await load(); } else { setFeedback(null); } })(); }, [penaltyLeft, feedback, supabase, attempt?.id, load]);
  async function begin() { setBusy(true); setMessage(""); const { data: value, error } = await supabase.rpc("start_today_daily_challenge"); setBusy(false); if (error) return setMessage(error.message); const normalizedAttempt = { ...value, id: value?.id || value?.attempt_id }; if (!normalizedAttempt.id) return setMessage("Não foi possível iniciar a tentativa. Atualize a página e tente novamente."); setData((previous) => ({ ...previous, attempt: normalizedAttempt })); setNow(Date.now()); setStarted(true); }
  async function answer(selected) { if (!current || busy || feedback) return; if (!attempt?.id) return setMessage("A tentativa não foi vinculada. Clique em Sair e em Retomar desafio."); setBusy(true); setMessage(""); const { data: result, error } = await supabase.rpc("submit_daily_challenge_answer", { p_attempt_id: attempt.id, p_question_id: current.id, p_selected_answer: selected }); setBusy(false); if (error) return setMessage(`Não foi possível registrar a resposta: ${error.message}`); if (!result) return setMessage("O servidor não retornou a correção. Tente novamente."); if (result.correct && result.is_last) { window.setTimeout(load, 420); return; }
    if (result.correct) { setFeedback({ correct: true, position: result.position }); window.setTimeout(() => setFeedback(null), 420); setData((previous) => ({ ...previous, attempt: { ...previous.attempt, current_position: result.position, correct_count: result.correct_count } })); return; }
    setData((previous) => ({ ...previous, attempt: { ...previous.attempt, current_position: result.position, next_allowed_at: result.next_allowed_at } })); setFeedback({ ...result, correct: false });
  }
  if (!data) return <div className="center">Carregando desafio…</div>;
  if (!data.available) return <section className="card challenge-intro"><span className="eyebrow">DESAFIO DIÁRIO</span><h1>Ainda não há desafio publicado</h1><p>Volte mais tarde para testar seus conhecimentos em cinco questões rápidas.</p><button onClick={onBack}>Voltar ao início</button></section>;
  if (attempt?.status === "completed") return <section className="challenge-result card"><span className="eyebrow">DESAFIO DIÁRIO</span><h1>Desafio concluído</h1><strong>{formatChallengeTime(attempt.elapsed_ms)}</strong><p>{attempt.correct_count}/5 corretas · {attempt.error_count} erro{attempt.error_count === 1 ? "" : "s"}</p><DailyChallengeRanking supabase={supabase} compact /><div><button onClick={onRanking}>Ver classificação</button><button className="light" onClick={onBack}>Voltar ao início</button></div></section>;
  if (!started) return <section className="challenge-intro card"><span className="eyebrow">DESAFIO DIÁRIO</span><h1>{attempt ? "Retomar desafio" : "Desafio diário"}</h1><p>5 questões rápidas de Sim ou Não.</p><div className="challenge-directions"><span>← <b>NÃO</b></span><span><b>SIM</b> →</span></div><small>Arraste a carta para a esquerda para NÃO e para a direita para SIM.</small><small>Você possui apenas uma tentativa válida para o ranking.</small><button disabled={busy} onClick={begin}>{attempt ? "Retomar desafio" : "Iniciar desafio"}</button><button className="link-button" onClick={onBack}>Voltar</button></section>;
  if (attempt?.status === "awaiting_completion" && !feedback) return <section className="challenge-intro card"><span className="eyebrow">DESAFIO DIÁRIO</span><h1>Correção em andamento</h1><p>Aguarde a penalidade da última resposta terminar.</p><button onClick={() => setFeedback({ awaiting_completion: true, next_allowed_at: attempt.next_allowed_at })}>Continuar</button></section>;
  return <section className={`challenge-play ${feedback ? (feedback.correct ? "flash-good" : "flash-bad") : ""}`}><div className="challenge-top"><button type="button" className="link-button" onClick={onBack}>← Sair</button><b><LineIcon name="timer" /> {formatChallengeTime(elapsed)}</b><span>{(attempt?.current_position || 0) + 1} de 5</span></div>{message && <p className="challenge-message" role="alert">{message}</p>}{feedback && !feedback.correct ? <div className="challenge-feedback error"><span>ERRADO</span><h2>Resposta correta: {feedback.correct_answer ? "SIM" : "NÃO"}</h2><p>{feedback.explanation || "Revise o fundamento aplicável."}</p>{feedback.legal_basis && <small>{feedback.legal_basis}</small>}<b>Próxima questão em {penaltyLeft}s</b></div> : <><div className="challenge-card" onPointerDown={(event) => { swipeStart.current = event.clientX; event.currentTarget.setPointerCapture?.(event.pointerId); }} onPointerUp={(event) => { const start = swipeStart.current; swipeStart.current = null; if (start !== null && Math.abs(event.clientX - start) > 55) answer(event.clientX > start); }} onPointerCancel={() => { swipeStart.current = null; }}><span>{feedback?.correct ? "CORRETO" : "SIM OU NÃO"}</span><h1>{current?.statement}</h1>{current?.discipline && <small>{[current.discipline, current.subject].filter(Boolean).join(" · ")}</small>}</div><div className="challenge-actions"><button type="button" className="no" disabled={busy} onClick={() => answer(false)}>← Não</button><button type="button" className="yes" disabled={busy} onClick={() => answer(true)}>Sim →</button></div>{feedback?.correct && <div className="challenge-correct">Correto</div>}</>}</section>;
}
function Performance({ questions, userId, supabase, goStudy }) { const answers = useAnswerHistory(supabase, userId); const metrics = answerMetrics(answers, questions); return <section className="card performance-page"><span className="eyebrow">DESEMPENHO</span><h1>Seu progresso</h1><p>Os dados abaixo são calculados a partir das suas respostas registradas.</p><div className="metric-grid"><MetricCard label="Respondidas" value={metrics.total} icon="◉" /><MetricCard label="Acertos" value={metrics.correct} icon="✓" /><MetricCard label="Taxa geral" value={`${metrics.rate}%`} icon="↗" /><MetricCard label="Média recente" value={`${metrics.recentRate}%`} icon="◌" /></div><h2>Evolução recente</h2><MiniChart answers={metrics.days} /><div className="performance-lines"><span>Melhor disciplina <b>{metrics.best?.name || "—"}</b></span><span>Disciplina que merece atenção <b>{metrics.focus?.name || "—"}</b></span></div><DailyChallengeRanking supabase={supabase} /><button onClick={goStudy}>Voltar aos estudos</button></section>; }
function UserProfile({ profile, session, supabase, onProfileSaved }) {
  ...
  async function resetCycles() { if (!window.confirm("Zerar todos os ciclos, respostas, acertos e erros? Esta ação não pode ser desfeita.")) return; setBusy(true); try { const { error } = await supabase.rpc("reset_my_study_history"); if (error) throw error; window.localStorage.removeItem(`caderno-questoes:study-cycle:${session.user.id}`); setMessage("Ciclos, respostas, acertos, erros e taxa de acerto foram zerados. Atualizando…"); window.setTimeout(() => window.location.reload(), 700); } catch (error) { setMessage(error.message); setBusy(false); } }
...
function parseChallengeBatch(value) {
  return value.replace(/\r/g, "").split(/(?=^\s*\d+\s*[.)])/m).map((block) => {
    const answer = block.match(/resposta\s*:\s*(sim|n[aã]o)/i);
    const statement = block.replace(/^\s*\d+\s*[.)]\s*/, "").replace(/resposta\s*:\s*(sim|n[aã]o)[\s\S]*$/i, "").trim();
    return answer && statement ? { id: crypto.randomUUID(), statement, correct_answer: /^sim$/i.test(answer[1]), discipline: "", subject: "", explanation: "", legal_basis: "" } : null;
  }).filter(Boolean);
}
function ChallengeAdmin({ supabase }) {
  const today = new Date().toLocaleDateString("en-CA"), [view, setView] = React.useState("bank"), [items, setItems] = React.useState([]), [challenges, setChallenges] = React.useState([]), [date, setDate] = React.useState(today), [selected, setSelected] = React.useState([]), [editing, setEditing] = React.useState(null), [form, setForm] = React.useState({ statement:"", correct_answer:true, discipline:"", subject:"", explanation:"", legal_basis:"", active:true }), [raw, setRaw] = React.useState(""), [batch, setBatch] = React.useState([]), [search, setSearch] = React.useState(""), [usage, setUsage] = React.useState("all"), [message, setMessage] = React.useState(""), [busy, setBusy] = React.useState(false);
  const load = React.useCallback(async () => { const [{ data: qs, error: qError }, { data: ds, error: dError }] = await Promise.all([supabase.from("challenge_questions").select("*").order("created_at", { ascending:false }), supabase.from("daily_challenges").select("*").order("challenge_date", { ascending:false })]); if (qError || dError) setMessage((qError || dError).message); setItems(qs || []); setChallenges(ds || []); }, [supabase]);
  React.useEffect(() => { load(); }, [load]);
  const currentChallenge = challenges.find((challenge) => challenge.challenge_date === date);
  React.useEffect(() => { let live = true; if (!currentChallenge) { setSelected([]); return undefined; } supabase.from("daily_challenge_questions").select("question_id,position").eq("challenge_id", currentChallenge.id).order("position").then(({ data, error }) => { if (!live) return; if (error) setMessage(error.message); else setSelected((data || []).map((entry) => entry.question_id)); }); return () => { live = false; }; }, [currentChallenge?.id, supabase]);
  function patchForm(key, value) { setForm((old) => ({ ...old, [key]: value })); }
  async function saveSingle(event) { event.preventDefault(); if (!form.statement.trim()) return setMessage("Informe a pergunta/afirmação."); setBusy(true); const payload = { ...form, statement:form.statement.trim(), discipline:form.discipline.trim() || null, subject:form.subject.trim() || null, explanation:form.explanation.trim() || null, legal_basis:form.legal_basis.trim() || null }; const request = editing ? supabase.from("challenge_questions").update(payload).eq("id", editing) : supabase.from("challenge_questions").insert(payload); const { error } = await request; setBusy(false); if (error) return setMessage(error.message); setMessage(editing ? "Questão atualizada." : "Questão de desafio salva."); setEditing(null); setForm({ statement:"", correct_answer:true, discipline:"", subject:"", explanation:"", legal_basis:"", active:true }); load(); }
  async function saveBatch() { const valid = batch.filter((item) => item.statement.trim()); if (!valid.length) return setMessage("Nenhuma questão válida para salvar."); setBusy(true); const { error } = await supabase.from("challenge_questions").insert(valid.map(({ id, ...item }) => ({ ...item, statement:item.statement.trim(), discipline:item.discipline || null, subject:item.subject || null, explanation:item.explanation || null, legal_basis:item.legal_basis || null }))); setBusy(false); if (error) return setMessage(error.message); setBatch([]); setRaw(""); setMessage(`${valid.length} questão(ões) salvas.`); load(); }
  function choose(id) { setSelected((current) => current.includes(id) ? current.filter((item) => item !== id) : current.length >= 5 ? (setMessage("O desafio deve conter exatamente 5 questões."), current) : [...current, id]); }
  function move(id, direction) { setSelected((list) => { const index = list.indexOf(id), next = index + direction; if (next < 0 || next >= list.length) return list; const copy = [...list]; [copy[index], copy[next]] = [copy[next], copy[index]]; return copy; }); }
  async function saveDraft() { if (currentChallenge?.status === "published") { setMessage("Desafio publicado não pode ser alterado."); return null; } if (selected.length !== 5) { setMessage(`Selecione 5 questões. Atualmente: ${selected.length}/5.`); return null; } setBusy(true); let challenge = currentChallenge; if (!challenge) { const { data, error } = await supabase.from("daily_challenges").insert({ challenge_date:date }).select().single(); if (error) { setBusy(false); setMessage(error.message); return null; } challenge = data; }
    const { error: deleteError } = await supabase.from("daily_challenge_questions").delete().eq("challenge_id", challenge.id); if (deleteError) { setBusy(false); setMessage(deleteError.message); return null; } const { error } = await supabase.from("daily_challenge_questions").insert(selected.map((question_id, index) => ({ challenge_id:challenge.id, question_id, position:index + 1 }))); setBusy(false); if (error) { setMessage(error.message); return null; } setMessage("Rascunho salvo com cinco questões."); load(); return challenge.id; }
  async function publish() { const challengeId = await saveDraft(); if (!challengeId) return; setBusy(true); const { error } = await supabase.rpc("publish_daily_challenge", { p_challenge_id:challengeId }); setBusy(false); if (error) return setMessage(error.message); setMessage("Desafio publicado. A composição agora está bloqueada."); load(); }
  async function removeItem(id) { if (!window.confirm("Excluir esta questão do banco de desafios?")) return; const { error } = await supabase.from("challenge_questions").delete().eq("id", id); if (error) setMessage(error.message); else { setMessage("Questão excluída."); load(); } }
  async function toggleAvailable(item) { const { error } = await supabase.from("challenge_questions").update({ administratively_available:!item.administratively_available }).eq("id", item.id); if (error) setMessage(error.message); else load(); }
  const filtered = items.filter((item) => (!search || `${item.id} ${item.statement} ${item.discipline || ""} ${item.subject || ""}`.toLowerCase().includes(search.toLowerCase())) && (usage === "all" || usage === "new" ? usage !== "new" || item.times_used === 0 : item.times_used > 0));
  const selectedItems = selected.map((id) => items.find((item) => item.id === id)).filter(Boolean);
  return <section className="challenge-admin"><span className="eyebrow">ADMINISTRAÇÃO</span><h1>Desafios diários</h1><p>Banco independente de afirmações “Sim ou Não”, com seleção manual de cinco perguntas por dia.</p><div className="challenge-admin-tabs"><button className={view === "bank" ? "active" : ""} onClick={() => setView("bank")}>Banco de questões</button><button className={view === "daily" ? "active" : ""} onClick={() => setView("daily")}>Desafios diários</button><button className={view === "ranking" ? "active" : ""} onClick={() => setView("ranking")}>Ranking</button></div>{message && <p className="form-message">{message}</p>}
    {view === "bank" && <><div className="challenge-bank-grid"><form className="card challenge-question-form" onSubmit={saveSingle}><h2>{editing ? "Editar questão" : "Cadastrar questão"}</h2><label>Pergunta/Afirmação *<textarea rows="4" value={form.statement} onChange={(event) => patchForm("statement", event.target.value)} required /></label><label>Resposta correta<select value={String(form.correct_answer)} onChange={(event) => patchForm("correct_answer", event.target.value === "true")}><option value="true">SIM</option><option value="false">NÃO</option></select></label><label>Disciplina<input value={form.discipline} onChange={(event) => patchForm("discipline", event.target.value)} /></label><label>Assunto<input value={form.subject} onChange={(event) => patchForm("subject", event.target.value)} /></label><label>Explicação<textarea rows="3" value={form.explanation} onChange={(event) => patchForm("explanation", event.target.value)} /></label><label>Fundamento legal<input value={form.legal_basis} onChange={(event) => patchForm("legal_basis", event.target.value)} /></label><label className="check-line"><input type="checkbox" checked={form.active} onChange={(event) => patchForm("active", event.target.checked)} /> Ativa</label><button disabled={busy}>Salvar questão</button>{editing && <button type="button" className="light" onClick={() => { setEditing(null); setForm({ statement:"", correct_answer:true, discipline:"", subject:"", explanation:"", legal_basis:"", active:true }); }}>Cancelar</button>}</form><section className="card challenge-bulk"><h2>Cadastro em lote</h2><p>Cole itens com “Resposta: SIM” ou “Resposta: NÃO”.</p><textarea rows="10" value={raw} onChange={(event) => setRaw(event.target.value)} placeholder={"1. A competência tributária pode ser delegada.\nResposta: NÃO\n\n2. A Administração Pública deve observar a eficiência.\nResposta: SIM"} /><button type="button" onClick={() => setBatch(parseChallengeBatch(raw))}>Revisar itens</button>{batch.length > 0 && <div className="batch-mini">{batch.map((item, index) => <div key={item.id}><input value={item.statement} onChange={(event) => setBatch((list) => list.map((entry) => entry.id === item.id ? { ...entry, statement:event.target.value } : entry))} /><select value={String(item.correct_answer)} onChange={(event) => setBatch((list) => list.map((entry) => entry.id === item.id ? { ...entry, correct_answer:event.target.value === "true" } : entry))}><option value="true">SIM</option><option value="false">NÃO</option></select><button className="danger compact" type="button" onClick={() => setBatch((list) => list.filter((entry) => entry.id !== item.id))}>Excluir</button></div>)}</div>}<button disabled={busy || !batch.length} type="button" onClick={saveBatch}>Salvar todas</button></section></div><div className="challenge-filters"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por texto ou ID" /><select value={usage} onChange={(event) => setUsage(event.target.value)}><option value="all">Todas</option><option value="new">Não utilizadas</option><option value="used">Já utilizadas</option></select></div><div className="challenge-question-list">{filtered.map((item) => <article className="card challenge-question-row" key={item.id}><div><b>#{String(item.id).slice(0, 8)}</b><small>{[item.discipline,item.subject].filter(Boolean).join(" · ") || "Sem classificação"}</small><p>{item.statement}</p><em className={item.correct_answer ? "answer-yes" : "answer-no"}>Resposta: {item.correct_answer ? "SIM" : "NÃO"}</em><small>{item.times_used ? `Usada ${item.times_used} vez(es) · última: ${item.last_used_at || "—"}` : "Nunca utilizada"}{!item.administratively_available && " · indisponível administrativamente"}</small></div><div><button className="light compact" onClick={() => { setEditing(item.id); setForm({ statement:item.statement, correct_answer:item.correct_answer, discipline:item.discipline || "", subject:item.subject || "", explanation:item.explanation || "", legal_basis:item.legal_basis || "", active:item.active }); window.scrollTo({ top:0, behavior:"smooth" }); }}>Editar</button><button className="light compact" onClick={() => toggleAvailable(item)}>{item.administratively_available ? "Marcar indisponível" : "Marcar disponível"}</button><button className="danger compact" onClick={() => removeItem(item.id)}>Excluir</button></div></article>)}</div></>}
    {view === "daily" && <section className="card challenge-builder"><div className="builder-head"><div><h2>Montar desafio</h2><p>Selecione exatamente cinco questões e defina a ordem que todos receberão.</p></div><label>Data<input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label></div><b className={selected.length === 5 ? "selection-ok" : ""}>{selected.length}/5 questões selecionadas {currentChallenge?.status === "published" && "· Publicado"}</b><div className="challenge-selection"><div><h3>Banco disponível</h3>{items.filter((item) => item.active).map((item) => <label className={`selectable-question ${selected.includes(item.id) ? "selected" : ""}`} key={item.id}><input disabled={currentChallenge?.status === "published"} type="checkbox" checked={selected.includes(item.id)} onChange={() => choose(item.id)} /><span><b>{item.correct_answer ? "SIM" : "NÃO"}</b>{item.statement}<small>{item.times_used ? `Já utilizada ${item.times_used} vez(es)` : "Nunca utilizada"}</small></span></label>)}</div><div className="selected-challenge"><h3>Ordem do desafio</h3>{selectedItems.map((item, index) => <div key={item.id}><b>{index+1}.</b><span>{item.statement}</span><button disabled={currentChallenge?.status === "published" || index === 0} className="light compact" onClick={() => move(item.id,-1)}>↑</button><button disabled={currentChallenge?.status === "published" || index === selectedItems.length-1} className="light compact" onClick={() => move(item.id,1)}>↓</button></div>)}{!selectedItems.length && <p>Selecione as cinco perguntas ao lado.</p>}</div></div><div className="builder-actions"><button disabled={busy || selected.length !== 5 || currentChallenge?.status === "published"} onClick={saveDraft}>Salvar rascunho</button><button disabled={busy || selected.length !== 5 || currentChallenge?.status === "published"} onClick={publish}>Publicar desafio</button></div><h3>Histórico</h3><div className="challenge-history">{challenges.map((challenge) => <button key={challenge.id} className={challenge.challenge_date === date ? "active" : ""} onClick={() => setDate(challenge.challenge_date)}><b>{challenge.challenge_date}</b><small>{challenge.status === "published" ? "Publicado" : challenge.status === "draft" ? "Rascunho" : "Cancelado"}</small></button>)}</div></section>}
    {view === "ranking" && <ChallengeAdminRanking supabase={supabase} challenges={challenges} />}
  </section>;
}
function ChallengeAdminRanking({ supabase, challenges }) { const [challengeId,setChallengeId] = React.useState(""), [rows,setRows] = React.useState([]); React.useEffect(() => { if (!challengeId) return setRows([]); supabase.from("challenge_attempts").select("*").eq("challenge_id",challengeId).order("elapsed_ms").then(({data}) => setRows(data || [])); }, [challengeId,supabase]); return <section className="card admin-challenge-ranking"><h2>Ranking administrativo</h2><select value={challengeId} onChange={(event) => setChallengeId(event.target.value)}><option value="">Selecione uma data</option>{challenges.filter((challenge) => challenge.status === "published").map((challenge) => <option key={challenge.id} value={challenge.id}>{challenge.challenge_date}</option>)}</select>{rows.map((row,index) => <div key={row.id}><b>{index+1}º</b><span>{row.user_id.slice(0,8)}</span><span>{formatChallengeTime(row.elapsed_ms)}</span><span>{row.correct_count}/5 · {row.error_count} erros</span></div>)}</section>; }
function CorrectionCreditsAdmin({ supabase }) {
  const [users, setUsers] = React.useState([]), [credits, setCredits] = React.useState([]), [requests, setRequests] = React.useState([]), [selected, setSelected] = React.useState(""), [search, setSearch] = React.useState(""), [notice, setNotice] = React.useState(""), [busy, setBusy] = React.useState(false);
  const load = React.useCallback(async () => { const [{ data: profileRows, error: profileError }, { data: creditRows, error: creditError }, { data: requestRows, error: requestError }] = await Promise.all([supabase.from("profiles").select("id,email,display_name").order("email"), supabase.from("essay_correction_credits").select("*").order("created_at", { ascending:false }), supabase.from("essay_correction_requests").select("*").order("created_at", { ascending:false })]); if (profileError || creditError || requestError) setNotice((profileError || creditError || requestError).message); setUsers(profileRows || []); setCredits(creditRows || []); setRequests(requestRows || []); }, [supabase]);
  React.useEffect(() => { load(); }, [load]);
  async function issue(userId, requestId = null) { if (!userId) return setNotice("Selecione um usuário."); setBusy(true); const token = `DQ-${crypto.randomUUID().replace(/-/g, "").slice(0, 10).toUpperCase()}`; const { error } = await supabase.from("essay_correction_credits").insert({ user_id:userId, token }); if (!error && requestId) await supabase.from("essay_correction_requests").update({ status:"approved", reviewed_at:new Date().toISOString() }).eq("id", requestId); setBusy(false); if (error) return setNotice(error.message); setNotice(`Token ${token} emitido para o usuário.`); load(); }
  async function review(request, status) { if (status === "approved") return issue(request.user_id, request.id); const { error } = await supabase.from("essay_correction_requests").update({ status:"rejected", reviewed_at:new Date().toISOString() }).eq("id", request.id); if (error) setNotice(error.message); else load(); }
  const visibleUsers = users.filter((user) => `${user.display_name || ""} ${user.email || ""}`.toLowerCase().includes(search.toLowerCase()));
  return <section className="credits-admin"><span className="eyebrow">CRÉDITOS DE CORREÇÃO</span><h1>Tokens por usuário</h1><p>Cada token libera uma correção adicional de discursiva acima do limite mensal.</p>{notice && <p className="form-message">{notice}</p>}<section className="card credit-issuer"><label>Usuário<select value={selected} onChange={(event) => setSelected(event.target.value)}><option value="">Selecione um usuário</option>{users.map((user) => <option key={user.id} value={user.id}>{user.display_name || user.email}</option>)}</select></label><button disabled={busy || !selected} onClick={() => issue(selected)}>Emitir 1 token</button></section>{requests.filter((request) => request.status === "pending").length > 0 && <section className="card credit-requests"><h2>Solicitações pendentes</h2>{requests.filter((request) => request.status === "pending").map((request) => { const user = users.find((item) => item.id === request.user_id); return <div key={request.id}><span><b>{user?.display_name || user?.email || "Usuário"}</b><small>{request.message || "Solicitou uma correção adicional."}</small></span><button disabled={busy} onClick={() => review(request,"approved")}>Aprovar + token</button><button className="light" disabled={busy} onClick={() => review(request,"rejected")}>Recusar</button></div>; })}</section>}<div className="credit-user-filter"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar usuário" /></div><section className="credit-user-list">{visibleUsers.map((user) => { const own = credits.filter((credit) => credit.user_id === user.id), unused = own.filter((credit) => !credit.used_at), used = own.filter((credit) => credit.used_at); return <article className="card" key={user.id}><div><b>{user.display_name || "Sem nome"}</b><small>{user.email}</small></div><div className="credit-count"><span><b>{unused.length}</b> disponíveis</span><span><b>{used.length}</b> usados</span><span><b>{own.length}</b> emitidos</span></div><button disabled={busy} className="light" onClick={() => issue(user.id)}>+ 1 token</button>{unused.length > 0 && <details><summary>Ver tokens disponíveis ({unused.length})</summary>{unused.map((credit) => <code key={credit.id}>{credit.token}</code>)}</details>}</article>; })}</section></section>;
}
...
function EssayLibrary({ supabase }) {
  const [items, setItems] = React.useState([]), [selected, setSelected] = React.useState([]), [editing, setEditing] = React.useState(null), [search, setSearch] = React.useState(""), [filter, setFilter] = React.useState(""), [notice, setNotice] = React.useState(""), [profiles, setProfiles] = React.useState([]), [requests, setRequests] = React.useState([]), [creditUser, setCreditUser] = React.useState(""), [generatedCredit, setGeneratedCredit] = React.useState(""), [savingEdit, setSavingEdit] = React.useState(false);
  ...
  async function saveEdit() {
    ...
    if (!String(editing.statement || "").trim()) return setNotice("O enunciado não pode ficar vazio.");
    setSavingEdit(true); setNotice("");
    const payload = {
      question_number: editing.question_number ? Number(editing.question_number) : null,
      discipline: String(editing.discipline || "").trim() || null,
      subjects: formatSubjects(editing.subjects),
      difficulty: editing.difficulty || "media",
      banca: String(editing.banca || "").trim() || null,
      ano: editing.ano ? Number(editing.ano) : null,
      concurso: String(editing.concurso || "").trim() || null,
      application_date: editing.application_date || null,
      total_points: Number(editing.total_points || 0),
      statement: String(editing.statement).trim(),
      answer_key_text: String(editing.answer_key_text || "").trim() || null,
      official_commentary: String(editing.official_commentary || "").trim() || null,
      is_hidden: Boolean(editing.is_hidden),
      updated_at: new Date().toISOString(),
    };
    const { error } = await supabase.from("essay_questions").update(payload).eq("id", editing.id);
    setSavingEdit(false);
    if (error) { setNotice(`Não foi possível salvar: ${error.message}`); return; }
    setEditing(null); setNotice("Alterações salvas com sucesso."); load();
  }
