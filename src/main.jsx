...
function DailyChallenge({ supabase, onBack, onRanking }) {
  ...
  async function begin() { setBusy(true); const { data: value, error } = await supabase.rpc("start_today_daily_challenge"); setBusy(false); if (error) return setMessage(error.message); setData((previous) => ({ ...previous, attempt: value })); setNow(Date.now()); setStarted(true); }
  async function answer(selected) { if (!current || busy || feedback) return; setBusy(true); const { data: result, error } = await supabase.rpc("submit_daily_challenge_answer", { p_attempt_id: attempt.id, p_question_id: current.id, p_selected_answer: selected }); setBusy(false); if (error) return setMessage(error.message); if (result.correct && result.is_last) { window.setTimeout(load, 420); return; }
  ...
  return <section className={`challenge-play ${feedback ? (feedback.correct ? "flash-good" : "flash-bad") : ""}`}><div className="challenge-top"><button className="link-button" onClick={onBack}>← Sair</button><b><LineIcon name="timer" /> {formatChallengeTime(elapsed)}</b><span>{(attempt?.current_position || 0) + 1} de 5</span></div>{feedback && !feedback.correct ? <div className="challenge-feedback error"><span>ERRADO</span><h2>Resposta correta: {feedback.correct_answer ? "SIM" : "NÃO"}</h2><p>{feedback.explanation || "Revise o fundamento aplicável."}</p>{feedback.legal_basis && <small>{feedback.legal_basis}</small>}<b>Próxima questão em {penaltyLeft}s</b></div> : <><div className="challenge-card" onPointerDown={(event) => event.currentTarget.dataset.startX = event.clientX} onPointerUp={(event) => { const start = Number(event.currentTarget.dataset.startX || event.clientX); if (Math.abs(event.clientX - start) > 70) answer(event.clientX > start); }}><span>{feedback?.correct ? "CORRETO" : "SIM OU NÃO"}</span><h1>{current?.statement}</h1>{current?.discipline && <small>{[current.discipline, current.subject].filter(Boolean).join(" · ")}</small>}</div><div className="challenge-actions"><button className="no" disabled={busy} onClick={() => answer(false)}>← Não</button><button className="yes" disabled={busy} onClick={() => answer(true)}>Sim →</button></div>{feedback?.correct && <div className="challenge-correct">Correto</div>}</>}</section>;
}
