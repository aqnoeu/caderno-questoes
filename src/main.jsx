  ...
  return (
    <main className="app-shell">
      <header className="app-header">
        ...
        {area === "user" && <nav className="user-nav">{[["inicio", "⌂", "Início"], ["estudos", "▤", "Questões"], ["discursivas", "✎", "Discursivas"], ["desempenho", "▥", "Desempenho"], ["perfil", "⚙", "Perfil"]].map(([id, icon, label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}><span aria-hidden="true">{icon}</span>{label}</button>)}</nav>}
      </header>
      {notice && (
