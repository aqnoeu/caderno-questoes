...
function useSystem2Automation(supabase) {
  const defaultUrl = `${import.meta.env.VITE_SUPABASE_URL || supabase.supabaseUrl || ""}/functions/v1/process-system2-queue`;
  const [settings, setSettings] = React.useState({ enabled:false, max_per_day:30, min_interval_minutes:5, function_url:defaultUrl });
  const [message, setMessage] = React.useState("");
  const load = React.useCallback(async () => {
    const { data, error } = await supabase.from("system2_ai_settings").select("enabled, max_per_day, min_interval_minutes, function_url, last_started_at").eq("id", true).maybeSingle();
    if (error) { setMessage(`Configure o banco da automação: ${error.message}`); return; }
    if (data) setSettings((current) => ({ ...current, ...data, function_url:data.function_url || defaultUrl }));
  }, [defaultUrl, supabase]);
  React.useEffect(() => { load(); }, [load]);
  async function save(patch = {}) {
    const next = { ...settings, ...patch, id:true, function_url:(patch.function_url ?? settings.function_url ?? defaultUrl).trim() };
    const { error } = await supabase.from("system2_ai_settings").upsert(next);
    if (error) { setMessage(`Não foi possível salvar a automação: ${error.message}`); return false; }
    setSettings((current) => ({ ...current, ...next })); setMessage(next.enabled ? "Automação ativada. O servidor verificará a fila a cada 5 minutos." : "Automação pausada.");
    return true;
  }
  return { settings, setSettings, message, setMessage, save, reload:load };
}
...
export function System2Filter({ supabase, onBack, openContent }) {
  ...
  const automation = useSystem2Automation(supabase);
  ...
  async function runNow() { if (!(await automation.save({ enabled:true }))) return; setRunning(true); automation.setMessage("Enviando a próxima questão para análise…"); const response = await supabase.functions.invoke("process-system2-queue", { body:{ trigger:"manual" } }); automation.setMessage(response.error ? `Não foi possível iniciar a análise: ${response.error.message}` : response.data?.reason || `Questão ${response.data?.question_number || ""} enviada ao Filtro de IA.`); setRunning(false); await Promise.all([reload(), automation.reload()]); }
  return <section className="system2-page"><button className="link-button" onClick={onBack}>← Voltar ao Sistema 2.0</button><span className="eyebrow">ETAPA 02</span><h1>Filtro de IA</h1><p className="system2-lead">Acompanhe a fila real de análise. As questões só seguem ao Controle de Conteúdo quando forem aprovadas.</p><section className="card system2-automation"><div><span className="eyebrow">AUTOMAÇÃO DO SERVIDOR</span><h2>{automation.settings.enabled ? "Fila ativa" : "Fila pausada"}</h2><p>Uma questão por vez, com limite diário e intervalo configuráveis. O navegador pode ser fechado: o servidor continua o trabalho.</p></div><div className="system2-automation-controls"><label className="switch-line"><input type="checkbox" checked={automation.settings.enabled} onChange={(event) => automation.save({ enabled:event.target.checked })} /> {automation.settings.enabled ? "Ativa" : "Pausada"}</label><label>Limite por dia<input type="number" min="1" max="500" value={automation.settings.max_per_day} onChange={(event) => automation.setSettings((current) => ({ ...current, max_per_day:Number(event.target.value) || 1 }))} onBlur={() => automation.save()} /></label><label>Intervalo (min.)<input type="number" min="1" max="1440" value={automation.settings.min_interval_minutes} onChange={(event) => automation.setSettings((current) => ({ ...current, min_interval_minutes:Number(event.target.value) || 1 }))} onBlur={() => automation.save()} /></label><button disabled={running} onClick={runNow}>{running ? "Analisando…" : "Analisar próxima agora"}</button></div><small>Função: {automation.settings.function_url || "configure após publicar a Edge Function"}</small>{automation.message && <p className="form-message">{automation.message}</p>}</section><div className="system2-tabs">{tabs.map((id) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}>{statusInfo[id].label} <b>{questions.filter((item) => item.status === id).length}</b></button>)}</div><div className="system2-toolbar"><p>{statusInfo[tab].detail}</p><button className="light compact" onClick={reload}>Atualizar dados</button></div>{loading ? <p className="empty-state">Carregando a fila…</p> : error ? <p className="form-message">Não foi possível carregar a fila: {error}</p> : selected.length ? <div className="system2-tracking-list">{selected.map((item) => <System2QuestionRow key={item.id} item={item} />)}</div> : <div className="card empty-state"><b>Nenhuma questão nesta etapa.</b><p>Quando uma questão for salva no Cadastro 2.0, ela aparecerá em “Pendentes de análise”.</p></div>}<div className="system2-note">A aprovação envia a questão automaticamente para o Controle de Conteúdo. Resultado incompleto, erro da IA ou baixa confiança seguem para revisão humana.</div><button className="light" onClick={openContent}>Ir para Controle de Conteúdo →</button></section>;
}
