...
function parseQuestionsV2(text) {
  ...
  return starts.map((start, index) => {
    ...
    return { id: crypto.randomUUID(), question_number: Number(start.match[1]), statement, alternatives, warnings, selected: true };
...
export function System2Cadastro({ supabase, onBack }) {
  const [metadata, setMetadata] = React.useState(emptyMetadata), [file, setFile] = React.useState(null), [drafts, setDrafts] = React.useState([]), [info, setInfo] = React.useState(null), [busy, setBusy] = React.useState(false), [notice, setNotice] = React.useState("");
  ...
  async function inspectFile(fileToRead) {
    ...
    try {
      const result = await extractPdfV2(fileToRead); const parsed = parseQuestionsV2(result.text);
      setFile(fileToRead); setInfo(result); setDrafts(parsed);
  ...
  async function save() {
    ...
    try {
      ...
      const rows = chosen.map((item) => ({ import_id:imported.id, question_number:item.question_number, statement:item.statement.trim(), alternatives:item.alternatives, extraction_warnings:item.warnings, status:"pending_ai", source_page_range:null, raw_payload:{ imported_file:file?.name || null } }));
