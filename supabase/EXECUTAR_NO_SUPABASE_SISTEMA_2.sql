...
create table if not exists public.questions_v2 (
  ...
  alternatives jsonb not null default '[]'::jsonb, answer_key_option text check (answer_key_option is null or answer_key_option in ('A','B','C','D','E','F','G','H','*')), extraction_warnings jsonb not null default '[]'::jsonb, source_page_range text,
