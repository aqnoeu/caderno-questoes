-- Sistema 2.0: pipeline isolado. Nenhuma tabela do acervo atual é alterada.
...
create table if not exists public.questions_v2 (
  ...
  -- Reservado para a etapa futura do Filtro de IA.
  discipline text,
  subjects jsonb not null default '[]'::jsonb,
  subtopics jsonb not null default '[]'::jsonb,
  legal_concepts jsonb not null default '[]'::jsonb,
...
create or replace function public.touch_sistema2_updated_at()
...
create trigger touch_question_imports_updated_at before update on public.question_imports
  for each row execute function public.touch_sistema2_updated_at();
drop trigger if exists touch_questions_v2_updated_at on public.questions_v2;
create trigger touch_questions_v2_updated_at before update on public.questions_v2
  for each row execute function public.touch_sistema2_updated_at();
